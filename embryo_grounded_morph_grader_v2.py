...
CFG = {
    ...
    "targets": ["EXP", "ICM", "TE"],
    "num_morph_tokens": 6,
    "output_dir": "./embryo_project/grounded_morph_grader",   # consolidated project folder
    "seed": 42,

    # --- same enhancement methodology as embryo_grader_finetune_v2.py, held
    #     IDENTICAL on purpose so architecture is the only variable that changes ---
    "focal_gamma": 2.0,
    ...
    "batch_size": 16, "accum_iter": 1, "encoder_lr": 1e-4, "head_lr": 3e-4, "min_lr": 1e-6,   # head_lr lowered from 1e-3: too aggressive was a likely collapse cause
...
# ============================================================
# ROBUST CSV + IMAGE RESOLUTION (unchanged utilities)
# ============================================================
...
# ============================================================
# TRANSFORMS + DATASET (identical multi-scale local/global aug)
# ============================================================
...
# ============================================================
# MODEL — GroundedMorphologyEncoder (6-token concat), same architecture
# as embryo_grounded_morph_grader.py. Only the training methodology
# below (loss/sampler/k-fold/both-inits) is new.
# ============================================================
...
class GroundedMorphologyEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((cls, morph, x), dim=1)
        x = self.encoder_norm(self.encoder(x))
        return x[:, 1:1 + self.num_morph, :]   # morphology tokens only

class EmbryoGroundedGrader(nn.Module):
    def __init__(self, cfg, num_classes, num_morph_tokens=6, mae_ckpt=None, init_from_mae=True):
        super().__init__()
        self.encoder = GroundedMorphologyEncoder(cfg, num_morph_tokens=num_morph_tokens)
...
# ============================================================
# Focal-CORAL + weighted sampler (identical to embryo_grader_finetune_v2.py)
# ============================================================
def focal_coral_loss(logits, levels, gamma=2.0):
    ...
    focal_w = (1 - p_t).clamp(min=1e-6) ** gamma
...
def evaluate(model, loader, targets, device):
    ...
    for t in targets:
        ...
        if len(yt) == 0:
            continue
        n_unique = len(np.unique(yp))   # FIX: track prediction diversity so collapse is
                                        # visible DURING training, not only discovered later
        out[t] = {'acc': float((yt == yp).mean()),
                  'macro_f1': float(f1_score(yt, yp, average='macro', zero_division=0)),
                  'qwk': float(cohen_kappa_score(yt, yp, weights='quadratic')) if len(np.unique(yt)) > 1 else 0.0,
                  'n': int(len(yt)), 'n_unique_preds': n_unique}
    return out
...
def train_one_fold(train_paths, train_labels, mae_cfg, num_classes, size, cfg, device,
                   te_loader, targets, init_from_mae, fold_idx, tag, maps):
    ...
    model = EmbryoGroundedGrader(mae_cfg, num_classes, num_morph_tokens=cfg['num_morph_tokens'],
                                 mae_ckpt=cfg['mae_checkpoint'], init_from_mae=init_from_mae).to(device)
    ...
    saved_path = None
    if init_from_mae:
        # FIX: save EVERY fold now, not just fold 1 -- fold 1 collapsing
        # shouldn't silently discard folds 2/3 if they trained fine. Pick
        # the best by real test QWK across kfold_summary.csv once all finish.
        saved_path = os.path.join(cfg['output_dir'], 'grounded_morph_v2_fold{}_mae_init.pth'.format(fold_idx))
        enc_sd = model.encoder.state_dict()
        torch.save({'model': model.state_dict(), 'encoder': enc_sd, 'mae_config': mae_cfg,
                    'num_morph_tokens': cfg['num_morph_tokens'], 'label_maps': maps,
                    'fold_test_qwk': {t: res_t['qwk'] for t, res_t in test_res.items()}}, saved_path)

    del model, optim
    gc.collect(); torch.cuda.empty_cache()
    return test_res, saved_path
...
def run_kfold_sweep(init_from_mae, train_df, maps, targets, mae_cfg, size, cfg, device, te_loader, num_classes):
    ...
    try:
        ...
        print("[KFOLD] StratifiedKFold failed -> falling back to plain KFold")
    ...
    for fi, (tr_idx, _) in enumerate(folds):
        ...
        res, saved_path = train_one_fold(tr_paths, tr_labels, mae_cfg, num_classes, size, cfg, device,
                                         te_loader, targets, init_from_mae, fi + 1, tag, maps)
        for t in targets:
            if t in res:
        ...
        # FIX: surface collapse the moment it happens, not weeks later when
        # someone runs inference and finds every prediction identical.
        for t in targets:
            if t in res and res[t]['n_unique_preds'] == 1:
                print("[{}] fold {}/{} *** COLLAPSED: {} head predicts only ONE class across the".format(tag, fi + 1, n_folds, t))
                print("             entire test set, regardless of true label. This checkpoint is unusable for {}. ***".format(t))
        if saved_path:
            print("[CHECKPOINT] saved representative encoder -> {}".format(saved_path))
...
def main():
    ...
    print("[ENCODER] embed_dim={} | real layers={} (depth//3) | morph tokens={}\n".format(
        mae_cfg['model']['embed_dim'], real_depth, cfg['num_morph_tokens']))
    ...
    print("=" * 78 + "\nRUN 1/{}: MAE-init k-fold sweep (grounded-morph, 6-token concat)\n".format(
        2 if cfg['run_both_inits'] else 1) + "=" * 78)
    ...
    if cfg['run_both_inits']:
        print("\n" + "=" * 78 + "\nRUN 2/2: from-scratch k-fold sweep\n" + "=" * 78)
    ...
    print("\n" + "=" * 78 + "\nFINAL SUMMARY — grounded-morph grader (mean +/- std QWK across {} folds)\n".format(cfg['n_folds']) + "=" * 78)
    ...
    print("\n  Grounded-morph MAE-init MEAN QWK = {:.3f}".format(mean_qwk_mae))
    print("  Compare directly to embryo_grader_finetune_v2.py's mean-pool MEAN QWK (same methodology, same folds).")
    print("  Whichever is higher tells you: does token-concat beat mean-pool, once training is made fair for both?")
    if res_scratch is not None:
        mean_qwk_scratch = summary[summary.init == 'from-scratch']['mean_qwk'].mean()
        print("  from-scratch MEAN QWK = {:.3f} (gap = {:+.3f})".format(mean_qwk_scratch, mean_qwk_mae - mean_qwk_scratch))
    ...
    plt.title('Grounded-morph grader (6-token concat): k-fold QWK by target'); plt.legend(); plt.grid(axis='y', alpha=0.3)
